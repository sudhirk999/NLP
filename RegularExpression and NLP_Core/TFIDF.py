...
word2count = {}
for i in range(len(datasets)):
    for word in nltk.word_tokenize(datasets[i]):
...
for word in freq_word:
    ...
    for data in datasets:
        if word in nltk.word_tokenize(data):
            word_count += 1
    word_idfs[word] = np.log((len(datasets)/word_count)+1)
...
for word in freq_word:
    ...
    for data in datasets:
        ...
        for w in nltk.word_tokenize(data):
            if w == word:
                word_count += 1
        doc_tf.append(word_count/len(nltk.word_tokenize(data)))
...
for word in tf_matrix.keys():
    tfidf = []
    for values in tf_matrix[word]:
        tfidf.append(values * word_idfs[word])
